...
public class BookInfoTest {
	...
	public void testGetRatingNonExistingBook() {
		...
		assertTrue(result.equals(Double.NaN));
	}
	
	@Test
	public void testGetRatingNonExistingBookInAPI() {
		bookInfo.rateBook("NotISBN", 5);
		double result = bookInfo.getRating("NotISBN");
		assertTrue(Math.abs(5 - result) < 1e-6);
		bookInfo.rateBook("NotISBN", 1);
		result = bookInfo.getRating("NotISBN");
		assertTrue(Math.abs(3 - result) < 1e-6);
	}
